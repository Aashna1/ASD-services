...
<input type="hidden" id="serviceName" name="serviceName" value="ComputerService"/>
...
$sql = "select * from mst_service_code  where service_code like 'C%' and service_code not in (select service_code from cart where userid = '".$userid."')";
...
$result = $conn->query($sql);
	...
	if ($result->num_rows > 0) 
	{
		...
		{			
				...
				if($isLoggedIn)
				{
					echo("<TD><TEXTAREA cols=50 id='probDesc".$counter."' name='probDesc".$counter."'></TEXTAREA></TD>");
					echo("<td><input type='button' id=btn".$counter." name=btn".$counter." value='Add to cart' onClick='doSubmit(\"$serviceCode\",\"".$counter."\")'></td>");
				}
				else
				{
					echo("<TD><TEXTAREA cols=50 disabled></TEXTAREA></TD>");
